...
workers.gatherAllChecks = function() {
  ...
  //SVM
  console.log('\nRunning workers.gatherAllChecks');

  _data.list('checks',function(err,checks) {

      //SVM
      console.log('value for checks: ',checks);
...
workers.validateCheckData = function(originalCheckData){
  ...
  //(if the workers have never seen this check)
  ...
  if(originalCheckData.id
      ...
      && originalCheckData.timeoutSeconds) {
          ...
          //SVM
          console.log('in lib/worker.js originalCheckData: ',originalCheckData);
...
//Perform check, send originalCheckData and the outcome of the check
...
workers.processCheckOutcome = function(originalCheckData,checkOutcome) {
  ...
  _data.update('checks',newCheckData.id,newCheckData,function(err) {
    if(!err) {
      ...
      if(alertWarranted) {
          console.log('in lib/workers.js alertWarranted Triggered')
...
//Alert the user to a change in a status
...
workers.loop = function() {
  setInterval(function(){
    console.log('launching workers.loop and workers.gatherAllChecks');
...
workers.rotateLogs = function() {
  //Lost all the non compressed log files
  _logs.list(false,function(err,logs)  {
      console.log('in workers.js workers.rotateLogs err: ',err);
  if(!err && logs && logs.length > 0) {
    logs.forEach(function(logName) {
        ...
        _logs.compress(logId,newFileId,function(err) {
          if(!err) {
            //Trunate the log file
            _logs.truncate(logId,function(err) {
              if(!err) {
                console.log("Success trunctating logFile");
              } else {
                console.log("Error truncting logFile");
              }
            }); 
          } else {
...
workers.init = function() {
  ...
  console.log('launching workers.init and workers.gatherAllChecks');
  workers.gatherAllChecks();
